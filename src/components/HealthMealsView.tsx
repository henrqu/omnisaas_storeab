...
export default function HealthMealsView({ onShowNotification }: HealthMealsViewProps) {
  ...
  const handleAddHealth = (e: React.FormEvent) => {
    ...
    if (isNaN(wNum) || wNum <= 0) {
      setHealthError('Por favor, insira um peso válido.');
      return;
    }
    if (systolic && isNaN(sysNum)) {
      setHealthError('Pressão sistólica inválida.');
      return;
    }
    if (diastolic && isNaN(diaNum)) {
      setHealthError('Pressão diastólica inválida.');
      return;
    }
    if (isNaN(sleepNum) || sleepNum < 0 || sleepNum > 24) {
      setHealthError('Horas de sono devem ser entre 0 e 24.');
      return;
    }
    setHealthError('');

    const newRec = LocalDatabase.addHealthRecord({
      date: new Date().toISOString().split('T')[0],
      weight: wNum,
      systolic: sysNum || null,
      diastolic: diaNum || null,
      sleep_hours: sleepNum,
      symptoms: symptoms.trim() || 'Nenhum',
    ...
    setSymptoms('');
    setNotes('');
    onShowNotification('Sinais Vitais Salvos', 'Registro de saúde diário armazenado!', 'success');
  };
  ...
  const handleAddPregnancy = (e: React.FormEvent) => {
    ...
    if (isNaN(week) || week <= 0 || week > 42) {
      setPregError('A semana gestacional deve ser entre 1 e 42.');
      return;
    }
    if (pregWeight && isNaN(weightVal)) {
      setPregError('Peso corporal inválido.');
      return;
    }
    setPregError('');
    ...
    if (!autoSize) {
      if (week <= 4) autoSize = 'Semente de papoula';
      else if (week <= 8) autoSize = 'Framboesa (aprox. 1.6 cm)';
      else if (week <= 12) autoSize = 'Ameixa (aprox. 5.4 cm)';
      else if (week <= 16) autoSize = 'Abacate (aprox. 11.6 cm)';
      else if (week <= 20) autoSize = 'Banana (aprox. 25.6 cm)';
      else if (week <= 24) autoSize = 'Espiga de milho (aprox. 30 cm)';
      else if (week <= 28) autoSize = 'Beringela (aprox. 37.6 cm)';
      else if (week <= 32) autoSize = 'Melão amarelo (aprox. 42.4 cm)';
      else if (week <= 36) autoSize = 'Melancia pequena (aprox. 47.4 cm)';
      else autoSize = 'Pronto para nascer!';
    }
    ...
    LocalDatabase.addPregnancyRecord({
      ...
      symptoms: pregSymptoms.trim() || 'Leves',
      baby_size_estimate: autoSize,
      doctor_notes: pregNotes.trim() || 'Desenvolvimento fisiológico normal.'
    });
    ...
    setPregNotes('');
    setPregSize('');
    onShowNotification('Registro Gestacional Salvo', `Semana ${week} logada com sucesso!`, 'success');
  };
  ...
  const handleAddMeal = (e: React.FormEvent) => {
    ...
    if (!foodName.trim()) {
      setMealError('Por favor, informe a descrição do alimento.');
      return;
    }
    if (isNaN(cal) || cal <= 0) {
      setMealError('As calorias devem ser um número positivo.');
      return;
    }
    setMealError('');
    ...
    setCarbs('');
    setFat('');
    onShowNotification('Refeição Logada', 'Alimento adicionado à sua dieta de hoje!', 'success');
  };

  const handleDeleteMeal = (id: string) => {
    const updated = LocalDatabase.deleteMeal(id);
    setMeals(updated);
    onShowNotification('Refeição Removida', 'Registro excluído.', 'info');
  };

  const handleDeleteHealth = (id: string) => {
    const updated = LocalDatabase.deleteHealthRecord(id);
    setHealthRecords(updated);
    onShowNotification('Registro Médico Removido', 'Histórico atualizado.', 'info');
  };
    ...
    <div className="space-y-6" id="health-meals-container">
      ...
      <div className="flex border-b border-slate-800" id="health-tabs">
        ...
        >
          ...
          <span>Gestação & Desenvolvimento</span>
        </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="tab-health-panel">
          ...
          <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6 h-fit">
            <h3 className="text-sm font-bold text-white tracking-tight mb-4 flex items-center">
              ...
              Logar Sinais Vitais do Dia
            </h3>
            <form onSubmit={handleAddHealth} className="space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Peso Corporal (kg)</label>
                <input 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Pressão Sistólica (max)</label>
                  <input 
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Pressão Diastólica (min)</label>
                  <input 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Horas de Sono</label>
                <input 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Sintomas (Se houver)</label>
                <input 
                  type="text" placeholder="Dor de cabeça leve, refluxo, dores musculares..."
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Anotações Médicas / Notas Gerais</label>
                <textarea 
                  rows={3} placeholder="Disposição, dores musculares, estresse, hidratação..."
          ...
          <div className="lg:col-span-2 bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
            <h2 className="text-sm font-bold text-white tracking-tight mb-4">Histórico Médico Recente</h2>
                ...
                <div key={r.id} className="bg-slate-950/40 border border-slate-850 p-4 rounded-xl relative">
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
                    <div className="bg-slate-900/60 p-2.5 rounded-lg border border-slate-800">
                      <span className="text-slate-500 block text-[9px] uppercase">Peso Corporal</span>
                      <span className="text-slate-200 font-bold">{r.weight} kg</span>
                    </div>
                    <div className="bg-slate-900/60 p-2.5 rounded-lg border border-slate-800">
                      <span className="text-slate-500 block text-[9px] uppercase">Pressão Arterial</span>
                      <span className="text-slate-200 font-bold">{r.systolic && r.diastolic ? `${r.systolic}/${r.diastolic}` : 'Não aferida'}</span>
                    </div>
                    <div className="bg-slate-900/60 p-2.5 rounded-lg border border-slate-800">
                      <span className="text-slate-500 block text-[9px] uppercase">Tempo de Sono</span>
                  ...
                  <div className="mt-3 text-xs space-y-1.5">
                    <p className="text-slate-300"><span className="text-slate-500 font-medium">Sintomas:</span> {r.symptoms}</p>
                    <p className="text-slate-400 leading-relaxed"><span className="text-slate-500 font-medium">Notas:</span> {r.notes}</p>
                  </div>
                </div>
              ))}

              {healthRecords.length === 0 && (
                <p className="text-xs text-slate-500 text-center py-12">Nenhum registro médico catalogado.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="tab-pregnancy-panel">
          ...
          <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6 h-fit">
            <h3 className="text-sm font-bold text-white tracking-tight mb-4 flex items-center">
              ...
              Novo Acompanhamento Gestacional
            </h3>
            <form onSubmit={handleAddPregnancy} className="space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Semana da Gestação</label>
                <input 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Peso da Gestante (kg) - Opcional</label>
                <input 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Estimativa de Tamanho do Bebê (Opcional)</label>
                <input 
                  type="text" placeholder="Deixe em branco para cálculo automático do tamanho"
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Sintomas Vivenciados</label>
                <input 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Anotações do Obstetra / Notas Gerais</label>
                <textarea 
          ...
          <div className="lg:col-span-2 bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
            <h2 className="text-sm font-bold text-white tracking-tight mb-4">Cronologia Gestacional e Desenvolvimento Fetal</h2>
            <div className="space-y-4 max-h-[550px] overflow-y-auto pr-1">
                ...
                <div key={p.id} className="bg-slate-950/40 border border-slate-850 p-5 rounded-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800 flex items-center space-x-2.5">
                      ...
                      <div>
                        <span className="text-slate-500 block text-[9px] uppercase font-bold">Fetal (Tamanho)</span>
                    ...
                    {p.weight && (
                      <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800 flex items-center space-x-2.5">
                        ...
                        <div>
                          <span className="text-slate-500 block text-[9px] uppercase font-bold">Peso Corporal</span>
                  ...
                  <div className="mt-4 text-xs space-y-2 border-t border-slate-800/40 pt-3">
                    <p className="text-slate-300"><span className="text-slate-500 font-semibold">Sintomas:</span> {p.symptoms}</p>
                    <p className="text-slate-400 leading-relaxed"><span className="text-slate-500 font-semibold">Instruções médicas:</span> {p.doctor_notes}</p>
                  </div>
                </div>
              ))}

              {pregnancyRecords.length === 0 && (
                <p className="text-xs text-slate-500 text-center py-12">Nenhuma semana gestacional logged.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="tab-meals-panel">
          ...
          <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6 h-fit">
            ...
            <form onSubmit={handleAddMeal} className="space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Refeição</label>
                <select 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Descrição do Alimento</label>
                <input 
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1">Calorias (kcal)</label>
                <input 
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Proteínas (g)</label>
                  <input 
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Carbos (g)</label>
                  <input 
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Gorduras (g)</label>
                  <input 
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-5">
              <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3">Ingestão Acumulada (Hoje)</h3>
              <div className="grid grid-cols-4 gap-4 text-center">
                <div className="bg-slate-950/55 p-3 rounded-xl border border-slate-850">
                  <span className="text-slate-400 text-[10px] block font-semibold uppercase">Calorias</span>
                  <span className="text-white text-lg font-bold">{meals.reduce((sum, m) => sum + m.calories, 0)} kcal</span>
                </div>
                <div className="bg-slate-950/55 p-3 rounded-xl border border-slate-850">
                  <span className="text-slate-400 text-[10px] block font-semibold uppercase">Proteínas</span>
                  <span className="text-emerald-400 text-lg font-bold">{meals.reduce((sum, m) => sum + m.protein, 0)}g</span>
                </div>
                <div className="bg-slate-950/55 p-3 rounded-xl border border-slate-850">
                  <span className="text-slate-400 text-[10px] block font-semibold uppercase">Carbos</span>
                  <span className="text-cyan-400 text-lg font-bold">{meals.reduce((sum, m) => sum + m.carbs, 0)}g</span>
                </div>
                <div className="bg-slate-950/55 p-3 rounded-xl border border-slate-850">
                  <span className="text-slate-400 text-[10px] block font-semibold uppercase">Gorduras</span>
            ...
            <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
              <h2 className="text-sm font-bold text-white tracking-tight mb-4">Diário de Refeições</h2>
              <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="bg-slate-850 text-slate-300 font-semibold text-[10px] uppercase px-2 py-0.5 rounded-full border border-slate-700/55">
                          {m.meal_type === 'breakfast' ? 'Café da Manhã' : m.meal_type === 'lunch' ? 'Almoço' : m.meal_type === 'dinner' ? 'Jantar' : 'Lanche'}
                        </span>
                ...
                {meals.length === 0 && (
                  <p className="text-xs text-slate-500 text-center py-12">Nenhum alimento catalogado hoje.</p>
                )}
              </div>
            </div>
